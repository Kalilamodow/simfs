 ...
 */
function verifyResourceName(name, customInvalidCharacters) {
    ...
    if (invalidCharacters.some(x => name.includes(x)))
        return false;
    if (name.trim().length == 0)
        return false; // is  (remove whitespace)
    if (name
        .replace(/s/g, "")
        .split("")
        .every(x => x == "."))
...
export { Directory, SFFile, verifyResourceName };
