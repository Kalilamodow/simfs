...
var HandledCommandReturn;
(function (HandledCommandReturn) {
    HandledCommandReturn[HandledCommandReturn["CONTINUE"] = 0] = "CONTINUE";
    HandledCommandReturn[HandledCommandReturn["EXIT"] = 1] = "EXIT";
    HandledCommandReturn[HandledCommandReturn["EXIT_ERROR"] = 2] = "EXIT_ERROR";
})(HandledCommandReturn || (HandledCommandReturn = {}));
let sfs;
// Python function aliases lol
const print = (m) => console.log(m);
const input = (m) => readlineSync.question(m);
var Commands;
(function (Commands) {
    Commands["LIST"] = "ls";
    Commands["CHANGE_DIR"] = "cd";
    Commands["CREATE_FILE"] = "cf";
    Commands["CREATE_DIR"] = "mkdir";
    Commands["DELETE"] = "delete";
    Commands["WRITE"] = "wr";
    Commands["PRINT_FILE"] = "cat";
    Commands["CLEAR"] = "clear";
    Commands["HELP"] = "help";
    Commands["BYTEREPR"] = "serialize";
    Commands["SAVE"] = "disksave";
})(Commands || (Commands = {}));
/**
 * Handles a command.
 *
...
const handleCommand = (cmd) => {
    ...
    if (cmd == Commands.LIST) {
        const cwd = sfs.cwd();
        const filelist = cwd.get().length > 0
...
commands:
  ...
  exit: exits the shell`);
        ...
        if (cmd.includes("--string")) {
            print(Array.from(file.serialize())
 ...
 */
const startShell = (start, print_hello = true) => {
    if (start instanceof SimulatedFilesystem)
        sfs = start;
    else
        sfs = new SimulatedFilesystem(start);
    if (print_hello)
        print("welcome to the simfs shell! type 'help' for a list of commands.");
    // main loop
    for (;;) {
        const command = input(`(${pathlib.normalize(sfs.cwd_path).replaceAll("\\", "/")})> `);
        const res = handleCommand(command);
        if (res == HandledCommandReturn.EXIT)
            break;
    }
};
// if running from "npm run" then start the shell
if (process.argv[2])
    if (process.argv[2].includes("run"))
        startShell();
